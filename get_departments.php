...
elseif ($faculty === 'Science') {
    if ($program_level === 'UG') {
        // Fetch the last admission ID from the database
        $prefix = 'UGSC';
        $admission_id = generateAdmissionID($con, 'science_admissions', $prefix);

        // Insert the form data into the admissions table
        $sql = "INSERT INTO `science_admissions` (
                    `admission_id`, `first_name`, `middle_name`, `last_name`, `email`, `phone`, `dob`, 
                    `address`, `category`, `physically_disabled`, `faculty_id`, `department_id`, `course_id`, 
                    `program_level`, `obtained_cgpa`, `total_cgpa`, 
                    `physics_marks`, `chemistry_marks`, `math_marks`, `total_marks`, `grand_total`, `percentage`
                ) VALUES (
                    '$admission_id', '$firstName', '$middleName', '$lastName', '$email', '$phone', '$dob', 
                    '$address', '$category', '$pwd', '$faculty_id', '$department_id', '$course_id', 
                    '$program_level', NULL, NULL, 
                    '$physics', '$chemistry', '$math', '$total', '$grand_total', '$percentage'
                )";
    ...
    elseif($program_level === 'PG') {
        $graduationMarks = $_POST['graduationMarks'];
...
elseif ($faculty === 'Pharmaceutical Technology') {
    if ($program_level === 'UG') {
        ...
        // Fetch the last admission ID from the database
        $prefix = 'UGPT';
        $admission_id = generateAdmissionID($con, 'pharmaceutical_admissions', $prefix);

        // Insert the form data into the admissions table
        $sql = "INSERT INTO `pharma_admissions` (
                    `admission_id`, `first_name`, `middle_name`, `last_name`, `email`, `phone`, `dob`, 
                    `address`, `category`, `physically_disabled`, `faculty_id`, `department_id`, `course_id`, 
                    `program_level`, `wbjee_score`, `wbjee_gmr`, `category_gmr`, `obtained_cgpa`, `total_cgpa`, 
                    `physics_marks`, `chemistry_marks`, `math_marks`, `total_marks`, `grand_total`, `percentage`
                ) VALUES (
                    '$admission_id', '$firstName', '$middleName', '$lastName', '$email', '$phone', '$dob', 
                    '$address', '$category', '$pwd', '$faculty_id', '$department_id', '$course_id', 
                    '$program_level', '$wbjeeScore', '$wbjeeGmr', '$wbjeeCategoryRank', NULL, NULL, 
                    '$physics', '$chemistry', '$math', '$total', '$grand_total', '$percentage'
                )";
    ...
    elseif ($program_level === 'PG') {
        ...
        $sql = "INSERT INTO `pharma_admissions` (
                    ...
                    '$admission_id', '$firstName', '$middleName', '$lastName', '$email', '$phone', '$dob', 
                    '$address', '$category', '$pwd', '$faculty_id', '$department_id', '$course_id', 
                    '$program_level', '$gpatScore', '$gpatRank', NULL, NULL, 
                    '$physics', '$chemistry', '$math', '$total', '$grand_total', '$percentage'
                )";
...
elseif ($faculty === 'Arts') {
    if ($program_level === 'UG') {
        ...
        // Insert the form data into the admissions table
        $sql = "INSERT INTO admissions (admission_id, first_name, middle_name, last_name, email, phone, dob, category, address, faculty_id, department_id, course_id, jeemain_score, jeemain_rank)
                VALUES ('$admission_id', '$firstName', '$middleName', '$lastName', '$email', '$phone', '$dob', '$category', '$address', '$faculty_id', '$department_id', '$course_id', '$jeemainScore', '$jeemainRank')";
    ...
    elseif ($program_level === 'PG') {
        ...
        // Insert the form data into the admissions table
        $sql = "INSERT INTO admissions (admission_id, first_name, middle_name, last_name, email, phone, dob, category, address, faculty_id, department_id, course_id, graduation_marks, graduation_percentage)
                VALUES ('$admission_id', '$firstName', '$middleName', '$lastName', '$email', '$phone', '$dob', '$category', '$address', '$faculty_id', '$department_id', '$course_id', '$graduationMarks', '$graduationPercentage')";
